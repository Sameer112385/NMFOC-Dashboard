...
export function RevenueWbsFilter({ rows }: { rows: RevenueRow[] }) {
  ...
  const totals = {
    sapActual: filteredRows.reduce((sum, row) => sum + (row.sap_actual_cost ?? 0), 0),
    pmSimulated: filteredRows.reduce((sum, row) => sum + (row.pm_pending_cost ?? 0), 0),
    actual: filteredRows.reduce((sum, row) => sum + row.actual_cost_to_date, 0),
    ...
    recognized: filteredRows.reduce((sum, row) => sum + row.recognized_revenue_to_date, 0),
    ...
    margin: filteredRows.reduce((sum, row) => sum + row.forecast_margin, 0),
  };
  ...
  return (
    <div className="space-y-4">
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        <label className="block">
          ...
          <input
            ...
            className="w-full rounded-2xl border border-line/70 bg-panel/70 px-4 py-3 text-sm text-text outline-none placeholder:text-muted/70 focus:border-accent/50"
          />
        </label>
        <label className="block">
          <span className="mb-2 block text-sm text-muted">Filter by Description</span>
          <DarkSelect
      ...
      <div className={`flex items-center justify-between gap-3 px-4 py-3 text-sm text-muted ${surfaceCard}`}>
        <span>
        ...
        <button
          ...
          className="rounded-lg border border-line/70 px-3 py-2 text-xs font-medium text-text hover:bg-panel2/80"
        >
          Clear filters
        </button>
      </div>
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-9">
        {[
          { label: 'SAP Actual Cost', value: formatCurrency(totals.sapActual) },
          { label: 'PM Simulated Cost', value: formatCurrency(totals.pmSimulated) },
          { label: 'Management Actual Cost', value: formatCurrency(totals.actual) },
          ...
          { label: 'Recognized Revenue', value: formatCurrency(totals.recognized) },
          { label: 'Remaining Revenue', value: formatCurrency(totals.remainingRevenue) },
          { label: 'Remaining Cost', value: formatCurrency(totals.remainingCost) },
          { label: 'Forecast Margin', value: formatCurrency(totals.margin) },
        ].map((item) => (
        ...
        <table className="min-w-[1200px] divide-y divide-line/20 text-sm">
          <thead className="bg-panel2/60 text-left text-muted">
            <tr>
              ...
              <th className="px-4 py-3">Last Cost Post</th>
              <th className="px-4 py-3">Last SO Date</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-line/20 bg-panel/55">
            {filteredRows.map((row) => (
              <tr key={row.wbs_code}>
                <td className="px-4 py-3 text-text">{row.project_id}</td>
                ...
                <td className="px-4 py-3 text-muted">{row.last_actual_posting_date ? new Date(row.last_actual_posting_date).toLocaleDateString() : '-'}</td>
                <td className="px-4 py-3 text-muted">{row.last_sales_order_date ? new Date(row.last_sales_order_date).toLocaleDateString() : '-'}</td>
              </tr>
            ))}
          </tbody>
        </table>
